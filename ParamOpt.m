...
lb0=[0.01,   2, 100,    -1, -500.0];
ub0=[0.10,   5, 600,     1,  500.0];
...
for Z=Z0:CPUS:size(N,1)
    ...
    if BEST_FITNESS1<10
        save(strcat('output/BETTER_SOLUTIONS_',num2str(Z),'.mat'),'Z','BEST_FITNESS1','BEST_SOLUTION1');
    end
    ...
    if BEST_FITNESS1<=best_val
        if BEST_FITNESS1<best_val && best_val<realmax
            ...
            save('output/OTHER_BEST_SOLUTIONS.mat','Z','BEST_FITNESS','BEST_SOLUTION');
    ...
    save('output/BEST_SOLUTION.mat','Z','BEST_SOLUTION','best_val');
end
